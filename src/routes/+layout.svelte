<script lang="ts">
	...
	onMount(async () => {
		...
		if ($page.url.pathname == "/" || $page.url.pathname=="/about") appState.loadedTables = true;
		...
		if (res.ok) {
			...
			appState.feeds = fs;
			appState.metrics = dat.d.orologies
		}
		appState.loadedTables = true;
		if (authState.user) {
			let d;
			try {
				d =
				...
				appState.userFeeds = d;
				let r = await pb.collection('rations').getFullList();
				// Cast record model to State interface
				appState.userRations = r as unknown as State[];
			} catch (err) {
				console.log(err);
				appState.userFeeds = [];
			}
	...
	$effect(() => {
		authState.user = pb.authStore.model || null;
	});
				...
				<div class="print:hidden flex justify-between text-xs sm:text-base sm:space-x-10 sm:pr-2">
						<TabGroup class="text-slate-400" active="font-bold text-white"  hover="hover:variant-ghost-surface">
							...
							<TabAnchor class="" selected={$page.url.pathname.includes("newration")}
								><button aria-label="Solve Menu" use:popup={popupSolve}>Επίλυση</button>
						...
						<div class="flex flex-col">
							{#if authState.user}
							<button aria-label="User Menu" class="  rounded-full" use:popup={popupUser}
							...
							<div in:fly={{x:200,duration:500}} class="p-2 rounded-lg variant-filled-secondary absolute text-lg top-0 left-0 z-10" data-popup="popupUser">
								...
								<ol>
									...
									<li>
										<button class="hover:underline hover:bg-teal-400 hover:rounded-lg hover:shadow-lg" onclick={logout} >Αποσύνδεση</button>
									</li>
								</ol>
							</div>
							{/if}</div>
