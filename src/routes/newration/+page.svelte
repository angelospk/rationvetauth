...
<div class="hide-scrollbar">
	{#if mounted}
		<div in:fly={{ y: -200, duration: 1000 }} class="print:hidden">
			<p class="text-xl md:text-3xl lg:text-4xl mb-2 heading">
				Χειροκίνητος Υπολογισμός Σιτηρεσίου
			</p>
			<div class="justify-center inline-flex mb-4">
				<a href="/newration/linear"
					><div
					...
					>
						<p class="md:text-lg">Αυτόματη Επίλυση</p>
						<p class="text-xs -mb-1">με χρήση γραμμικού προγραμματισμού</p>
					</div></a
				>
			</div>
			<hr />
	...
	{#if mounted}
		...
		>
			...
			{#if !loadedTable}
				<EditableTable bind:rationName bind:producerName bind:currentState />
			{:else}
			...
			<AccordionSaveShare bind:currentState bind:currentDate></AccordionSaveShare>
		</form>
		<hr class="my-3" />
